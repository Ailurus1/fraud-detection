...
class Dashboard:
    def __init__(
        self, bootstrap_servers=["localhost:9095", "localhost:9096"], max_rows=15
    ):
            ...
            with gr.Row():
                ...
                def get_transactions():
                    ...
                    header_row = (
                        ...
                        + "".join(
                            [
                                f"<th style='padding: 8px; background-color: silver'><b>{h}</b></th>"
                                for h in headers
                            ]
                        )
                        + "</tr>"
                ...
                gr.HTML(value=get_transactions, every=1)
            ...
            with gr.Row():
                ...
                with gr.Column():
                    with gr.Row():
                        ...
                        gr.Textbox(
                            value=get_fraud_counts,
                            label="Total Fraud Transactions Detected",
                            every=4,
                        )
    ...
    def start(self, open_browser: bool = True):
        ...
        interface.launch(
            inbrowser=open_browser, server_name="0.0.0.0", server_port=7860, share=False
        )
...
def main(bootstrap_servers, max_rows, open_browser):
    ...
    ui = Dashboard(bootstrap_servers=bootstrap_servers.split(","), max_rows=max_rows)
