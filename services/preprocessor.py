...
class TransactionPreprocessor:
    def __init__(
        self, metadata_path, bootstrap_servers=["localhost:9095", "localhost:9096"]
    ):
    ...
    def preprocess_transaction(self, transaction):
        transaction["type_encoded"] = int(
            self.label_encoder.transform([transaction["type"]])[0]
        )
        ...
        while True:
            ...
            try:
                ...
                self.producer.produce(
                    ...
                    json.dumps(
                        {"original": transaction, "processed": processed}
                    ).encode("utf-8"),
...
def main(metadata_path, bootstrap_servers):
    ...
    preprocessor = TransactionPreprocessor(
        metadata_path=metadata_path, bootstrap_servers=bootstrap_servers.split(",")
    )
    preprocessor.start()
