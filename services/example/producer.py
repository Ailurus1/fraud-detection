...
class TransactionProducer:
    def __init__(
        self, bootstrap_servers=["localhost:9095", "localhost:9096"], frequency=1.0
    ):
    ...
    def load_data(self):
        self.data = (
            df.drop(["isFraud", "isFlaggedFraud"]).sample(n=50, seed=seed).to_dicts()
        )
...
def create_producer(
    bootstrap_servers=["localhost:9095", "localhost:9096"], frequency=1.0
):
